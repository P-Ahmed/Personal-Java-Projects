...
public class Class_one extends Student {
    final double monthly_fees = 300;

    public Class_one(int st_count, int fees_paid_st) {
    ...
    public void reminder(){
        if(st_count > fees_paid_st){
            System.out.println("Remind the remaining class one students to pay fees.");
        }
            ...
            System.out.println("1.Kindly check your input\n"
                    +"2.Count total students of class one carefully\n"
                    +"3.Count the receipt of fees paid class one students carefully.");
        }
    }
    public void classFeesReport(){
        System.out.printf("Fees taken from class one %d students out of %d students.\n",fees_paid_st,st_count);
        monthlyFeesPaid();
        reminder();
        System.out.println("");
    }
